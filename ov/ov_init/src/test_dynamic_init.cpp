...
// 定义当按下 ctrl-c (SIGINT) 时调用的函数
void signal_callback_handler(int signum) { std::exit(signum); }

// 取自 ov_eval/src/alignment/AlignUtils.h
// 计算最佳偏航角
  ...
  while (sim.ok()) {
    ...
    if (hascam) {
      ...
      if (success) {
        ...
        // 首先让我们将真实状态与 IMU 状态对齐
        // 注意：IMU 偏差不需要在这里用位置偏航对齐来校正...
